...
function CartFragment() {
  ...
  const handleCartChange = (index, value) => {
    const inputValue = parseInt(value);
    const { stock, discountPrice } = cart[index].product;

    const updateCart = [...cart];

    if (!isNaN(inputValue) && inputValue >= 1 && inputValue <= stock) {
      updateCart[index].totalCart = inputValue;
      updateCart[index].totalPrice =
        updateCart[index].totalCart * discountPrice;
    } else if (isNaN(inputValue) || inputValue < 1) {
      updateCart[index].totalCart = "";
    } else if (inputValue > stock) {
      updateCart[index].totalCart = stock;
      updateCart[index].totalPrice =
        updateCart[index].totalCart * discountPrice;
    }
